...
class TagsRepository extends GetxController {
  ...
  void getTraits(List<Trait> a) async {
    isTraitReady.value = false;

    int hairId = 0;
    int eyesId = 0;
    log("test" + a[0].id.replaceAll('i', ''));
    for (int i = 0; i < a.length; i++) {
      if (hairColor.contains(int.parse(a[i].id.replaceAll('i', '')))) {
        hairId = (int.parse(a[i].id.replaceAll('i', '')));
      } else if (eyesColor.contains(int.parse(a[i].id.replaceAll('i', '')))) {
        eyesId = (int.parse(a[i].id.replaceAll('i', '')));
      }
    }
